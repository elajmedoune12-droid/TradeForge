...
function RuleCard({ rule, onToggle, onDelete, onEdit }) {
  ...
  return (
    <div className={`rounded-2xl p-4 border transition-all ${!rule.active ? 'opacity-50' : ''}`}
      style={{
        background: rule.active ? 'rgba(22,27,34,0.8)' : 'rgba(16,20,28,0.6)',
        borderColor: rule.active ? `${color}25` : 'rgba(255,255,255,0.06)',
        borderLeft: rule.active ? `3px solid ${color}` : '3px solid rgba(255,255,255,0.08)',
      }}>
      <div className="flex items-start gap-3">
        <button onClick={() => onToggle(rule.id, rule.active)}
          ...
          {rule.active
            ...
            : <XCircle size={18} className="text-forge-muted" />}
        </button>
        <div className="flex-1 min-w-0">
          <p className={`text-sm leading-relaxed ${rule.active ? 'text-white' : 'text-forge-muted line-through'}`}>
            {rule.text}
          </p>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={() => onEdit(rule)}
            className="w-7 h-7 rounded-lg flex items-center justify-center text-forge-muted hover:text-white hover:bg-white/5 transition-all">
            <Pencil size={12} />
          </button>
          {confirmDelete ? (
            <div className="flex gap-1">
              <button onClick={() => { onDelete(rule.id); setConfirmDelete(false) }}
              ...
              <button onClick={() => setConfirmDelete(false)}
                className="w-7 h-7 rounded-lg flex items-center justify-center text-forge-muted hover:bg-white/5 transition-all">
                <X size={12} />
              </button>
            </div>
          ) : (
            <button onClick={() => setConfirmDelete(true)}
              className="w-7 h-7 rounded-lg flex items-center justify-center text-forge-muted hover:text-forge-red hover:bg-forge-red/10 transition-all">
              <Trash2 size={12} />
            </button>
          )}
        </div>
...
function RuleForm({ initial, onSave, onCancel, saving }) {
  ...
  return (
    <div className="rounded-2xl p-4 mb-4 border"
      style={{ background: 'rgba(16,20,28,0.95)', borderColor: 'rgba(247,183,49,0.2)', boxShadow: '0 0 24px rgba(247,183,49,0.06)' }}>
      <p className="text-sm font-medium mb-3">{initial ? 'Modifier la règle' : 'Nouvelle règle'}</p>
      <textarea
        value={text}
      ...
      <div className="flex gap-2">
        <button onClick={() => onSave({ text: text.trim(), category })}
...
function TabBacktest({ user }) {
  const [cycles, setCycles]           = useState([])   // { id, goal_hours, started_at, ended_at, sessions[] }
  ...
  // ── Modifier objectif du cycle actuel ────────────────────
  ...
  const handleNewCycle = async () => {
    ...
    if (!h || h <= 0) return
    // Clore le cycle actuel
    if (currentCycle) {
      await supabase.from('backtest_cycles')
...
useEffect(() => {
  ...
  if (!currentCycle) return (
    <div className="text-center py-16">
      ...
      <p className="text-sm text-white mb-1">Démarrez votre premier cycle de backtest</p>
      <p className="text-xs text-forge-muted mb-5">Définissez un objectif d'heures et suivez votre progression jour par jour.</p>
      <div className="flex gap-2 max-w-xs mx-auto">
        ...
        <button onClick={async () => {
          ...
          if (data) setCurrentCycle({ ...data, sessions: [] })
          setGoalInput('')
        }} disabled={!goalInput} className="btn-primary disabled:opacity-40">Démarrer</button>
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      {/* ── Cycle actuel — objectif + progression ── */}
      <div className="rounded-2xl p-4 mb-4"
        style={{ background: 'rgba(16,20,28,0.9)', border: `1px solid ${done ? 'rgba(46,160,67,0.3)' : 'rgba(247,183,49,0.2)'}` }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Target size={16} className="text-forge-accent" />
            <span className="text-sm font-medium text-white">
              Cycle #{cycles.length + 1}
              <span className="text-[10px] text-forge-muted ml-2">depuis {fmtDate(currentCycle.started_at)}</span>
            </span>
          </div>
          {!done && (
            <button onClick={() => { setGoalInput(String(currentCycle.goal_hours)); setShowGoalEdit(true) }}
              className="flex items-center gap-1 text-xs text-forge-muted hover:text-forge-accent transition-colors">
        ...
        <div className="grid grid-cols-3 gap-2 mt-3">
          <div className="text-center">
            <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-0.5">Fait</p>
            <p className="text-lg font-mono font-semibold text-forge-accent">{fmtMin(totalMin)}</p>
          </div>
          <div className="text-center">
            <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-0.5">Progression</p>
            <p className={`text-lg font-mono font-semibold ${done ? 'text-forge-green' : 'text-white'}`}>{progress}%</p>
          </div>
          <div className="text-center">
            <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-0.5">Restant</p>
            <p className="text-lg font-mono font-semibold text-forge-muted">{done ? '✓' : fmtMin(leftMin)}</p>
          </div>
        </div>
      </div>
      ...
      {done && !showNewCycle && (
        <div className="rounded-2xl p-4 mb-4 flex items-center gap-3"
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-forge-muted mt-0.5">{currentCycle.goal_hours}h de backtest complétées.</p>
          </div>
      ...
      {showNewCycle && (
        <div className="rounded-2xl p-4 mb-4"
          style={{ background: 'rgba(16,20,28,0.95)', border: '1px solid rgba(247,183,49,0.25)' }}>
          <p className="text-sm font-medium text-white mb-1">Nouvel objectif</p>
          <p className="text-xs text-forge-muted mb-3">Le cycle actuel sera archivé. Vos sessions sont conservées.</p>
          <div className="flex gap-2">
      ...
      {!done && (
        <div className="rounded-2xl p-4 mb-5"
          style={{ background: 'rgba(22,27,34,0.8)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <p className="text-xs text-forge-muted uppercase tracking-widest mb-3">Ajouter des minutes</p>
          <div className="flex gap-2">
          ...
          <p className="text-[10px] text-forge-muted mt-2">Les minutes sont cumulées si vous ajoutez plusieurs fois la même date.</p>
        </div>
      )}

      {/* ── Sessions cycle actuel par semaines ── */}
      {weeks.length === 0 && !done ? (
        <div className="text-center py-10">
          <Timer size={24} className="text-forge-muted mx-auto mb-2 opacity-30" />
          <p className="text-xs text-forge-muted">Aucune session pour ce cycle. Ajoutez vos premières minutes.</p>
        </div>
      ) : (
        <div className="space-y-3 mb-8">
          {weeks.map((week, wi) => {
            ...
            return (
              <div key={week.monday} className="rounded-2xl overflow-hidden"
                style={{ border: `1px solid ${isDown ? 'rgba(248,81,73,0.2)' : 'rgba(255,255,255,0.08)'}`, background: isDown ? 'rgba(248,81,73,0.04)' : 'rgba(16,20,28,0.7)' }}>
                <button className="w-full flex items-center justify-between px-4 py-3 text-left"
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-forge-muted mt-0.5">{week.sessions.length} session{week.sessions.length > 1 ? 's' : ''}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <p className="text-sm font-mono font-semibold text-white">{fmtMin(wMin)}</p>
                      {diffMin !== null && (
                    ...
                    {isExp ? <ChevronUp size={14} className="text-forge-muted" /> : <ChevronDown size={14} className="text-forge-muted" />}
                  </div>
                </button>
                {isExp && (
                  <div className="px-4 pb-3 space-y-1.5 border-t" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                    {week.sessions.map(s => {
                      const d = new Date(s.date + 'T12:00:00')
                      return (
                        <div key={s.id} className="flex items-center justify-between py-2 px-3 rounded-xl"
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-[10px] text-forge-muted w-8 font-mono">{DAYS_FR[d.getDay()]}</span>
                            <span className="text-xs text-forge-muted">{fmtDate(s.date)}</span>
                          </div>
                          <div className="flex items-center gap-3">
      ...
      {cycles.length > 0 && (
        <div>
          <p className="text-[10px] text-forge-muted uppercase tracking-widest px-1 mb-3">
            ...
            {[...cycles].reverse().map((cycle, ci) => {
              ...
              return (
                <div key={cycle.id} className="rounded-2xl overflow-hidden"
                  style={{ border: '1px solid rgba(255,255,255,0.06)', background: 'rgba(16,20,28,0.5)' }}>
                  <button className="w-full flex items-center justify-between px-4 py-3 text-left"
                    onClick={() => setExpandedCycles(e => ({ ...e, [cycle.id]: !isExp }))}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0"
                        style={{ background: cycDone ? 'rgba(46,160,67,0.15)' : 'rgba(255,255,255,0.05)' }}>
                        <span className="text-xs font-mono font-semibold"
                          style={{ color: cycDone ? '#3fb950' : '#8B949E' }}>#{cycNum}</span>
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      <p className="text-sm font-mono font-medium text-forge-muted">{fmtMin(cycMin)}</p>
                      {isExp ? <ChevronUp size={14} className="text-forge-muted" /> : <ChevronDown size={14} className="text-forge-muted" />}
                    </div>
                  </button>

                  {isExp && (
                    <div className="px-4 pb-3 border-t space-y-2" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                      ...
                      <div className="space-y-1.5 mt-2">
                        {cycWeeks.map(week => (
                          <div key={week.monday} className="flex items-center justify-between py-1.5 px-3 rounded-xl"
                            ...
                            <p className="text-xs font-mono text-forge-muted">{fmtMin(weekMin(week))}</p>
                          </div>
                        ))}
                      </div>
                    </div>
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { setEditingRule(null); setShowForm(v => !v) }}
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <div className="rounded-2xl p-3 text-center"
          ...
          <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-1">Respect</p>
        ...
        <div className="rounded-2xl p-3 text-center"
          ...
          <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-1">Respecté</p>
        ...
        <div className="rounded-2xl p-3 text-center"
          ...
          <p className="text-[10px] text-forge-muted uppercase tracking-wide mb-1">Violations</p>
      ...
      {totalTrades > 0 && (
        <div className="rounded-2xl px-4 py-3 mb-4"
          style={{ background: 'rgba(16,20,28,0.8)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="h-2.5 bg-forge-border rounded-full overflow-hidden flex">
          ...
          <div className="flex justify-between text-[10px] text-forge-muted mt-1.5">
      ...
      {rules.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-sm text-forge-muted mb-1">Aucune règle définie</p>
          <p className="text-xs text-forge-muted/60 mb-5">Définissez vos règles pour suivre et améliorer votre discipline.</p>
        ...
        <div className="space-y-2">
          {filtered.filter(r => r.active).length > 0 && (
            <>
              <p className="text-[10px] text-forge-muted uppercase tracking-widest px-1 mb-2">
          ...
          {filtered.filter(r => !r.active).length > 0 && (
            <>
              <p className="text-[10px] text-forge-muted uppercase tracking-widest px-1 mt-4 mb-2">
...
function TabErrors({ trades }) {
  ...
  const stats = useMemo(() => {
    ...
    trades.forEach(t => {
      ...
      if (h?.main_error) {
        hindsightMap[h.main_error] = (hindsightMap[h.main_error] || 0) + 1
      }
    })
    ...
    <div>
          ...
          <div className="space-y-3">
            {stats.patterns.map((p, i) => (
              <div key={i} className="card">
                ...
                <div className="h-1.5 bg-forge-border rounded-full overflow-hidden mb-2">
                  <div className={`h-full rounded-full ${p.lossRate >= 60 ? 'bg-forge-red' : 'bg-forge-accent'}`}
                    style={{ width: `${p.lossRate}%` }} />
                </div>
                <p className="text-xs text-forge-muted">{p.losses} pertes sur {p.total} occurrences</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {stats.emotions.length > 0 && (
        <div className="mb-6">
          ...
          <div className="card divide-y divide-forge-border">
            {stats.emotions.map(e => (
              <div key={e.name} className="py-3 first:pt-0 last:pb-0">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <div className="flex items-center gap-3 text-xs text-forge-muted font-mono">
                    ...
                    <span className={`font-medium ${e.lossRate >= 60 ? 'text-forge-red' : e.lossRate >= 40 ? 'text-forge-accent' : 'text-forge-muted'}`}>
                      {e.lossRate}% SL
                    </span>
                  </div>
                </div>
                <div className="h-1 bg-forge-border rounded-full overflow-hidden">
                  <div className={`h-full rounded-full ${e.lossRate >= 60 ? 'bg-forge-red/60' : e.lossRate >= 40 ? 'bg-forge-accent/60' : 'bg-forge-green/60'}`}
            ...
            {stats.hindsightErrors.map(({ label, count }) => (
              <div key={label}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-white truncate flex-1">{label}</span>
  ...
  return (
    <div className="page">
      <div className="mb-5">
        ...
        <p className="text-xs text-forge-muted">Règles, erreurs & backtest</p>
      </div>
