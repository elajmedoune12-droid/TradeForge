...
function Lightbox({ images, startIndex, onClose }) {
  ...
  const content = (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 99999, background: '#000', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 'calc(env(safe-area-inset-top) + 10px)', paddingBottom: '10px', paddingLeft: 16, paddingRight: 16, flexShrink: 0, background: 'rgba(0,0,0,0.9)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
...
function DetailPanel({ item, onClose, onDelete, onOpenImage }) {
  ...
  const content = (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(6px)', cursor: 'pointer' }} />
      <div onClick={e => e.stopPropagation()} style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: 760, maxHeight: '88vh', overflowY: 'auto', background: 'rgb(11,15,22)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 20, boxShadow: '0 40px 100px rgba(0,0,0,0.8)' }}>
        <div style={{ position: 'sticky', top: 0, zIndex: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'rgb(11,15,22)', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(46,160,67,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><BookMarked size={15} color="#2EA043" /></div>
            <div>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'white', margin: 0, lineHeight: 1.3 }}>{format(parseISO(item.created_at), "d MMMM yyyy 'à' HH:mm", { locale: fr })}</p>
              {images.length > 0 && <p style={{ fontSize: 11, color: '#8B949E', margin: 0 }}>{images.length} image{images.length > 1 ? 's' : ''}</p>}
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <button onClick={handleDelete} disabled={deleting} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 10, cursor: 'pointer', background: 'rgba(248,81,73,0.12)', border: '1px solid rgba(248,81,73,0.35)', color: '#F85149', fontSize: 12, fontWeight: 600, opacity: deleting ? 0.5 : 1, transition: 'all 0.15s' }}><Trash2 size={13} />{deleting ? 'Suppression…' : 'Supprimer'}</button>
            <button onClick={onClose} style={{ width: 34, height: 34, borderRadius: 10, cursor: 'pointer', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: '#8B949E', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={15} /></button>
          </div>
        </div>
        <div style={{ padding: '20px 24px' }}>
          {(item.timeframes?.length > 0 || item.markets?.length > 0) && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 24, marginBottom: 20 }}>
              {item.timeframes?.length > 0 && <div><p style={{ fontSize: 10, color: '#8B949E', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>⏱ Timeframes</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{item.timeframes.map(tf => <span key={tf} style={{ padding: '4px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: 'rgba(247,183,49,0.12)', color: '#F7B731', border: '1px solid rgba(247,183,49,0.3)' }}>{tf}</span>)}</div></div>}
              {item.markets?.length > 0 && <div><p style={{ fontSize: 10, color: '#8B949E', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>🌐 Marchés</p><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{item.markets.map(m => <span key={m} style={{ padding: '4px 12px', borderRadius: 8, fontSize: 12, fontWeight: 600, background: 'rgba(46,160,67,0.12)', color: '#2EA043', border: '1px solid rgba(46,160,67,0.3)' }}>{m}</span>)}</div></div>}
            </div>
          )}
          {item.notes && <div style={{ borderRadius: 12, padding: '14px 18px', marginBottom: 24, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}><p style={{ fontSize: 10, color: '#8B949E', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Notes</p><p style={{ fontSize: 13, color: 'white', lineHeight: 1.8, whiteSpace: 'pre-wrap', margin: 0 }}>{item.notes}</p></div>}
          {images.length > 0 && Object.entries(byTF).map(([tf, imgs]) => (
            <div key={tf} style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 11, fontFamily: 'monospace', fontWeight: 700, marginBottom: 10, color: tf === 'Sans timeframe' ? '#8B949E' : '#F7B731', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{tf}</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(200px, 100%), 1fr))', gap: 10 }}>
                {imgs.map((img, i) => (
                  <button key={i} onClick={() => onOpenImage(img.globalIdx)} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', aspectRatio: '16/9', border: '1px solid rgba(255,255,255,0.1)', cursor: 'pointer', padding: 0, background: 'none', transition: 'transform 0.15s, border-color 0.15s' }}
                    ...
                    onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.1)'; e.currentTarget.querySelector('.ov').style.opacity = '0' }}>
                    <img src={img.url} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                    <div className="ov" style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity 0.15s' }}><ZoomIn size={22} color="white" /></div>
                  </button>
                ))}
              </div>
            </div>
...
function HindsightCard({ item, onClick }) {
  ...
  return (
    <div onClick={onClick} className="card-hover group flex gap-4 cursor-pointer active:scale-[0.99]">
      <div style={{ flexShrink: 0, width: 76, height: 76, borderRadius: 12, overflow: 'hidden', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)' }}>
        {preview ? <img src={preview} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><BookMarked size={18} color="rgba(139,148,158,0.35)" /></div>}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 11, color: '#8B949E', marginBottom: 6 }}>{format(parseISO(item.created_at), 'd MMM yyyy', { locale: fr })}</p>
        {item.timeframes?.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 5 }}>{item.timeframes.slice(0, 5).map(tf => <span key={tf} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 6, fontWeight: 600, background: 'rgba(247,183,49,0.1)', color: '#F7B731' }}>{tf}</span>)}</div>}
        {item.markets?.length > 0 && <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: 5 }}>{item.markets.slice(0, 3).map(m => <span key={m} style={{ fontSize: 10, padding: '2px 7px', borderRadius: 6, fontWeight: 600, background: 'rgba(46,160,67,0.1)', color: '#2EA043' }}>{m}</span>)}</div>}
        {item.notes && <p style={{ fontSize: 12, color: '#8B949E', lineHeight: 1.5, margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{item.notes}</p>}
        {images.length > 0 && <p style={{ fontSize: 10, color: 'rgba(139,148,158,0.45)', marginTop: 5, display: 'flex', alignItems: 'center', gap: 3 }}><Image size={9} /> {images.length} image{images.length > 1 ? 's' : ''}</p>}
      </div>
    </div>
  )
}
...
function HindsightGoals({ items }) {
  ...
  return (
    <>
      {showHistory && (
        <>
          <div className="fixed inset-0 z-40" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(6px)' }} onClick={() => setShowHistory(false)} />
          <div className="fixed z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-sm rounded-2xl overflow-hidden"
            ...
            <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b" style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
              <p className="text-sm font-semibold text-white">Historique des objectifs</p>
              <button onClick={() => setShowHistory(false)} className="text-forge-muted hover:text-white"><X size={16} /></button>
            </div>
            <div className="overflow-y-auto p-3 space-y-3" style={{ maxHeight: 'calc(80vh - 60px)' }}>
              {cycles.length === 0 ? (
                <p className="text-xs text-forge-muted text-center py-8">Aucun objectif complété pour l'instant.</p>
              ) : (
                [...cycles].reverse().map((c, i) => {
                  ...
                  return (
                    <div key={c.id} className="rounded-xl p-3"
                      style={{ background: allAchieved ? 'rgba(46,160,67,0.06)' : 'rgba(255,255,255,0.03)', border: `1px solid ${allAchieved ? 'rgba(46,160,67,0.2)' : 'rgba(255,255,255,0.07)'}` }}>
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">{allAchieved ? '🏆' : '📋'}</span>
                          <div>
                            <p className="text-xs font-semibold text-white">Cycle #{cycles.length - i}</p>
                            <p className="text-[10px] text-forge-muted">{c.startedAt} → {c.endedAt}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                            style={{ background: allAchieved ? 'rgba(46,160,67,0.15)' : 'rgba(255,255,255,0.05)', color: allAchieved ? '#2EA043' : '#8B949E' }}>
                            {allAchieved ? '✓ Atteint' : 'Incomplet'}
                          </span>
                          <button
                            onClick={() => {
                              const newCycles = cycles.filter(x => x.id !== c.id)
                              setCycles(newCycles)
                              saveCycles(newCycles, currentCycle)
                            }}
                            className="w-6 h-6 rounded-lg flex items-center justify-center text-forge-muted hover:text-forge-red hover:bg-forge-red/10 transition-all">
                            <Trash2 size={11} />
                          </button>
                        </div>
                      </div>
                        ...
                        {cGoals.map(g => (
                          <div key={g.label}>
                            <div className="flex justify-between text-[10px] mb-0.5">
                              <span className="text-forge-muted">{g.label}</span>
      ...
      <div className="rounded-2xl p-4 mb-5"
        style={{ background: 'rgba(16,20,28,0.9)', border: '1px solid rgba(46,160,67,0.2)', boxShadow: '0 0 24px rgba(46,160,67,0.04)' }}>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-medium text-white">Objectifs d'étude</p>
          </div>
          <div className="flex items-center gap-2">
            {cycles.length > 0 && (
              <button onClick={() => setShowHistory(true)} className="text-xs text-forge-muted hover:text-white transition-colors flex items-center gap-1">
                <Clock size={11} /> Historique ({cycles.length})
              </button>
            )}
            {currentCycle && !editing && (
              <button onClick={() => setEditing(true)} className="text-xs text-forge-muted hover:text-white transition-colors flex items-center gap-1">
                <Pencil size={11} />
              </button>
            )}
          </div>
        ...
        {editing && (
          <div className="space-y-3 mb-4">
            <p className="text-xs text-forge-muted">{currentCycle ? 'Modifier les objectifs' : 'Définir vos objectifs'}</p>
            <div className="grid grid-cols-3 gap-2">
              {[
                ...
                <div key={f.key}>
                  <p className="text-[10px] text-forge-muted mb-1">{f.label}</p>
            ...
            <p className="text-[10px] text-forge-muted">Laissez vide les objectifs à ignorer.</p>
        ...
        {!editing && currentCycle && goals.length > 0 && (
          <>
                  ...
                  <div key={i} className="flex flex-col items-center gap-1.5">
                    <div className="relative" style={{ width: 70, height: 70 }}>
                      <svg width={70} height={70} style={{ transform: 'rotate(-90deg)' }}>
                        <circle cx={35} cy={35} r={29} fill="none" stroke="rgba(255,255,255,0.07)" strokeWidth={7} />
                    ...
                    <p className="text-[10px] text-forge-muted font-medium">{g.label}</p>
                    <p className="text-[10px]" style={{ color: done ? '#2EA043' : '#8B949E' }}>{g.current}{g.unit} / {g.goal}{g.unit}</p>
                  </div>
                )
              })}
            </div>
            <div className="space-y-2 pt-3 border-t mb-3" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
              {goals.map((g, i) => {
                const p = Math.min(100, Math.round((g.current / g.goal) * 100))
                const done = g.current >= g.goal
                return (
            ...
            {allDone && !showNewCycle && (
              <div className="rounded-xl p-3 flex items-center gap-3" style={{ background: 'rgba(46,160,67,0.08)', border: '1px solid rgba(46,160,67,0.25)' }}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] text-forge-muted mt-0.5">Prêt pour un nouveau cycle ?</p>
                </div>
            ...
            {showNewCycle && (
              <div className="rounded-xl p-3" style={{ background: 'rgba(16,20,28,0.95)', border: '1px solid rgba(247,183,49,0.25)' }}>
                <p className="text-xs font-medium text-white mb-1">Terminer ce cycle ?</p>
                <p className="text-[10px] text-forge-muted mb-3">Il sera archivé dans l'historique. Vous pourrez définir de nouveaux objectifs.</p>
                <div className="flex gap-2">
        ...
        {!editing && !currentCycle && (
          <div className="text-center py-4">
            <p className="text-sm text-forge-muted mb-3">Aucun objectif défini</p>
...
export default function HindsightsList() {
  ...
  if (loading) return (
    <div className="page space-y-4">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="h-9 w-24 rounded-xl animate-pulse" style={{ background: 'rgba(255,255,255,0.07)' }} />
      </div>
      {[...Array(5)].map((_, i) => (
        <div key={i} className="card flex gap-4 animate-pulse">
          <div className="rounded-xl flex-shrink-0" style={{ width: 76, height: 76, background: 'rgba(255,255,255,0.07)' }} />
  ...
  return (
    <div className="page animate-slide-up">
      ...
      {selected && !lightbox && <DetailPanel item={selected} onClose={() => setSelected(null)} onDelete={handleDelete} onOpenImage={(idx) => openImage(selected.images || [], idx)} />}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-forge-muted">{items.length} analyse{items.length !== 1 ? 's' : ''} de marché</p>
        </div>
        <button onClick={() => navigate('/hindsights/new')} className="btn-primary flex items-center gap-1.5"
              ...
              <div className="flex flex-wrap gap-1.5">
                {['', ...allTF].map(tf => (
                  <button key={tf} onClick={() => setFilterTF(tf)} className="px-3 py-1 rounded-lg text-xs font-medium transition-all"
                    style={filterTF === tf ? { background: 'rgba(247,183,49,0.15)', color: '#F7B731', border: '1px solid rgba(247,183,49,0.3)' } : { background: 'rgba(255,255,255,0.04)', color: '#8B949E', border: '1px solid rgba(255,255,255,0.08)' }}>
                    {tf || 'Tous'}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {['', ...allMkt].map(m => (
                  <button key={m} onClick={() => setFilterMkt(m)} className="px-3 py-1 rounded-lg text-xs font-medium transition-all"
                    style={filterMkt === m ? { background: 'rgba(46,160,67,0.15)', color: '#2EA043', border: '1px solid rgba(46,160,67,0.3)' } : { background: 'rgba(255,255,255,0.04)', color: '#8B949E', border: '1px solid rgba(255,255,255,0.08)' }}>
                    {m || 'Tous'}
                  </button>
                ))}
              </div>
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20">
          <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center" style={{ background: 'rgba(46,160,67,0.1)', border: '1px solid rgba(46,160,67,0.2)' }}>
            <BookMarked size={28} className="text-forge-green/60" />
          </div>
          <p className="text-forge-muted text-sm mb-1">{items.length === 0 ? "Aucun hindsight pour l'instant." : 'Aucun résultat.'}</p>
          {items.length === 0 && (
            <>
              <p className="text-forge-muted/50 text-xs mb-5">Analysez le marché librement, sans lier à un trade.</p>
              <button onClick={() => navigate('/hindsights/new')} className="btn-primary inline-flex items-center gap-2"
                style={{ background: '#2EA043', boxShadow: '0 0 20px rgba(46,160,67,0.3)' }}>
      ...
      {filtered.length > 0 && <p className="text-xs text-center text-forge-muted mt-5">{filtered.length} hindsight{filtered.length > 1 ? 's' : ''}</p>}
    </div>
  )
}